...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/landing/components/ui/tooltip";
            ...
            <TabsContent value="monthly" className="grid grid-cols-1 lg:grid-cols-3 gap-5 w-full md:gap-8 flex-wrap max-w-5xl mx-auto pt-6">
                {PLANS.map((plan) => (
                    <Card
                        key={plan.name}
                        className={cn(
                            "flex flex-col w-full border-border rounded-xl bg-black",
                            plan.name === "Pro" ? "border-2 border-orange-500 shadow-[0px_4px_20px_rgba(249,115,22,0.5)]" : "shadow-[0px_2px_10px_rgba(255,255,255,0.2)]"
                        )}
                    >
                        ...
                        <CardContent className="pt-6 space-y-4">
                            {plan.features.map((feature, index) => (
                                <div key={index} className="flex items-center gap-2">
                                    <CheckCircleIcon className="text-orange-600 w-4 h-4" />
                                    <TooltipProvider>
                        ...
                        <CardFooter className="w-full mt-auto">
                            <Link
                                href={plan.btn.href}
                                style={{ width: "100%" }}
                                className={buttonVariants({ className: plan.name === "Pro" && "bg-orange-600 hover:bg-orange-500/80 text-white" })}
                            >
                                {plan.btn.text}
                            </Link>
                        </CardFooter>
                    ...
                    >
                            ...
                            <h5 className="text-3xl font-semibold flex items-end">
                                ...
                                {plan.name !== "Free" && (
                                    <motion.span
                                        ...
                                        className="px-2 py-0.5 ml-2 rounded-md bg-orange-500 text-foreground text-sm font-medium"
                                    >
                                        -12%
                                    </motion.span>
                                )}
                        ...
                        <CardFooter className="w-full mt-auto">
                            <Link
                                href={plan.btn.href}
                                style={{ width: "100%" }}
                                className={buttonVariants({ className: plan.name === "Pro" && "bg-orange-500 hover:bg-orange-500/80 text-white" })}
                            >
                                {plan.btn.text}
                            </Link>
                        </CardFooter>
